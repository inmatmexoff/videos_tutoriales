...
import { Home, Search, Plus, Layers, User, LogOut } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <nav className="md:hidden fixed bottom-3 left-3 right-3 z-40">
      <div className="relative h-16">
        <div className="absolute inset-0 bg-card/95 backdrop-blur-md border shadow-lg rounded-full" />

        {centerPercent !== null && (
          <>
            {/* Halo concéntrico: crea la muesca cóncava que abraza el círculo */}
            <div
              className="absolute bg-background rounded-full pointer-events-none transition-[left] duration-300 ease-out"
              style={{ left: `calc(${centerPercent}% - ${HALO_RADIUS}px)`, top: HALO_TOP, width: HALO, height: HALO }}
            />
            {/* Círculo flotante del item activo */}
            <div
              className="absolute flex items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg transition-[left] duration-300 ease-out"
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-start justify-between px-1" style={{ top: ROW_TOP }}>
          <button type="button" onClick={() => router.push('/')} className="flex flex-col items-center flex-1">
            <span className={cn("h-5 w-5 flex items-center justify-center text-muted-foreground", activeIndex === 0 && "opacity-0")}>
              <Home className="h-4 w-4" />
            </span>
            <span className={cn("text-[9px] mt-0 transition-colors", activeIndex === 0 ? "font-bold text-foreground" : "text-muted-foreground")}>Inicio</span>
          </button>

          <button type="button" onClick={handleToggleSearch} className="flex flex-col items-center flex-1">
            <span className={cn("h-5 w-5 flex items-center justify-center text-muted-foreground", activeIndex === 1 && "opacity-0")}>
              <Search className="h-4 w-4" />
            </span>
            <span className={cn("text-[9px] mt-0 transition-colors", activeIndex === 1 ? "font-bold text-foreground" : "text-muted-foreground")}>Buscar</span>
          </button>

          <button type="button" onClick={() => router.push('/upload')} className="flex flex-col items-center flex-1">
            <span className={cn("h-5 w-5 flex items-center justify-center text-muted-foreground", activeIndex === 2 && "opacity-0")}>
              <Plus className="h-4 w-4" />
            </span>
            <span className={cn("text-[9px] mt-0 transition-colors", activeIndex === 2 ? "font-bold text-foreground" : "text-muted-foreground")}>Nuevo</span>
          </button>

          <button type="button" onClick={() => router.push('/admin')} className="flex flex-col items-center flex-1">
            <span className={cn("h-5 w-5 flex items-center justify-center text-muted-foreground", activeIndex === 3 && "opacity-0")}>
              <Layers className="h-4 w-4" />
            </span>
            <span className={cn("text-[9px] mt-0 transition-colors", activeIndex === 3 ? "font-bold text-foreground" : "text-muted-foreground")}>Estructura</span>
          </button>
